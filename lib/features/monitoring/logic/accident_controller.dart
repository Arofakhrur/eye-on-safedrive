...
class AccidentController extends ChangeNotifier {
  ...
  double get _accidentThreshold =>
      PreferenceService().shockSensitivity; // dynamically from user preference
  ...
  double get currentMagnitude => (_isCheckingSpeed || _isAccidentDetected)
      ? _peakMagnitude
      : _currentMagnitude;
  ...
  void startMonitoring() {
    _accelSubscription = userAccelerometerEventStream().listen((
      ...
      _filteredMagnitude =
          (_alpha * rawMagnitude) + ((1.0 - _alpha) * _filteredMagnitude);
  ...
  Future<void> _verifyAccidentWithSpeedGate() async {
    ...
    notifyListeners();
    debugPrint(
      '⚠️ Guncangan terdeteksi (LPF: ${_currentMagnitude.toStringAsFixed(2)} m/s²). Menunggu 4 detik untuk verifikasi Speed-Gate...',
    );
    ...
    tiltSubscription = accelerometerEventStream().listen((
      ...
      if (totalG < DetectionConfig.minGravityMagnitude) return;
      ...
      if (tiltDeg > DetectionConfig.tiltThresholdDegrees) {
        tiltTriggered = true;
        debugPrint(
          '🔄 TILT DETECTED: ${tiltDeg.toStringAsFixed(1)}° — Motor rebah! Bypass Speed-Gate.',
        );
    ...
    _speedCheckTimer?.cancel();
    _speedCheckTimer = Timer(
      Duration(seconds: DetectionConfig.speedGateSeconds),
      () async {
        ...
        try {
          ...
          if (position != null) {
            ...
            if (speedKmH < DetectionConfig.speedGateThresholdKmH) {
              ...
              debugPrint(
                '✅ FALSE ALARM: Motor masih melaju stabil. SOS dibatalkan.',
              );
          ...
          debugPrint(
            'Gagal verifikasi kecepatan: $e. Memaksa SOS demi keamanan.',
          );
          _sensorDetectionLatencyMs = stopwatch.elapsedMilliseconds;
          _isAccidentDetected = true;
          _triggerAccidentResponse();
  ...
  Future<void> _triggerAccidentResponse() async {
    ...
    try {
      ...
      final audioPath =
          AppAssets.alarmAudioFiles[alarmSound] ??
          AppAssets.alarmAudioFiles.values.first;
